...
try:
    ...
    st.info("Check if 'diabetes_model.pkl' and 'scaler.pkl' are in the app folder.")
    st.stop()
...
if st.button('Predict'):
    ...
    if prediction[0] == 1:
        st.error(f'The model predicts that you are likely to have diabetes. (Risk probability: {proba:.1%})')
        st.info("**Consult a healthcare professional** for proper diagnosis and management.")
    else:
        st.success(f'The model predicts that you are unlikely to have diabetes. (Risk probability: {proba:.1%})')
